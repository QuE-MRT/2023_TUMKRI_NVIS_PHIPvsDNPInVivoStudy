# Authors: Wolfgang Gottwald, wolfgang.gottwald@tum.de
...
def find_max_sig_rep(dnp_bssfp_data,phip_bssfp_data):
    ...
    if phip_bssfp_data.shape[0] == 1:
        ...
        logger.critical("phip_bssfp_data.shape[0] is larger than 1, this function only works for single echo data")
    if phip_bssfp_data.shape[5] == 1:
        pass
    else:
        logger.critical("phip_bssf_data.shape[5] is larger than 1, this function only works for single channel data")

    if dnp_bssfp_data.shape[0] == 1:
        pass
    else:
        logger.critical("dnp_bssfp_data.shape[0] is larger than 1, this function only works for single echo data")
    if dnp_bssfp_data.shape[5] == 1:
        pass
    else:
        logger.critical("dnp_bssfp_data.shape[5] is larger than 1, this function only works for single channel data")
    ...
    ax[0].set_title('PHIP = '+str(phip_max_sign_rep)+' // DNP = '+str(dnp_max_sign_rep))
    ...
    ax[1].set_title('PHIP')
    ax[2].set_title('DNP')
    fig.suptitle('Summed over all slices and displayed for the rep with the highest signal')
    return dnp_images[:,:,:,dnp_max_sign_rep],phip_images[:,:,:,phip_max_sign_rep]
...
def apply_sig_range(dnp_bssfp_data,phip_bssfp_data,signal_range_dict):
    ...
    return np.abs(np.sum(dnp_images[:,:,:,dnp_sig_reps[0]:dnp_sig_reps[1]],axis=3)),np.abs(np.sum(phip_images[:,:,:,phip_sig_reps[0]:phip_sig_reps[1]],axis=3))
...
def calculate_ssi(dnp_bssfp_data,dnp_bssfp,phip_bssfp_data,phip_bssfp):
    ...
    for idx in range(dnp_bssfp_data.shape[2]):
        phip_slice = np.abs((phip_bssfp_data[:, :, idx]) / np.mean(phip_bssfp_data[:, :, idx]))
        dnp_slice = np.abs((dnp_bssfp_data[:, :, idx]) / np.mean(dnp_bssfp_data[:, :, idx]))
        ...
        ssi_cor.append(ssim(phip_slice, dnp_slice, data_range=np.max(max_vals) - np.min(min_vals)))
    ...
    for idx in range(dnp_bssfp_data.shape[1]):
        ...
        ssi_sag.append(ssim(phip_slice, dnp_slice, data_range=np.max(max_vals) - np.min(min_vals)))
    ...
    for idx in range(dnp_bssfp_data.shape[0]):
        ...
        ssi_ax.append(ssim(phip_slice, dnp_slice, data_range=np.max(max_vals) - np.min(min_vals)))
    ...
    structural_sim_indices.update({"3D": ssim(phip_3D_meaned, dnp_3D_meaned,
                                              data_range=np.max([dnp_3D_meaned, phip_3D_meaned]) - np.min(
                                                  [dnp_3D_meaned, phip_3D_meaned]))})

    max_snr_slice_cor_dnp = np.argmin(np.abs(snr_per_sl_cor_dnp - np.max(snr_per_sl_cor_dnp)))
    max_snr_slice_ax_dnp = np.argmin(np.abs(snr_per_sl_ax_dnp - np.max(snr_per_sl_ax_dnp)))
    max_snr_slice_sag_dnp = np.argmin(np.abs(snr_per_sl_sag_dnp - np.max(snr_per_sl_sag_dnp)))

    max_snr_slice_cor_phip = np.argmin(np.abs(snr_per_sl_cor_phip - np.max(snr_per_sl_cor_phip)))
    max_snr_slice_ax_phip = np.argmin(np.abs(snr_per_sl_ax_phip - np.max(snr_per_sl_ax_phip)))
    max_snr_slice_sag_phip = np.argmin(np.abs(snr_per_sl_sag_phip - np.max(snr_per_sl_sag_phip)))

    structural_sim_indices.update({"coronal_best_slice_dnp": ssi_cor[max_snr_slice_cor_dnp]})
    structural_sim_indices.update({"sagittal_best_slice_dnp": ssi_sag[max_snr_slice_sag_dnp]})
    structural_sim_indices.update({"axial_best_slice_dnp": ssi_ax[max_snr_slice_ax_dnp]})

    structural_sim_indices.update({"coronal_best_slice_phip": ssi_cor[max_snr_slice_cor_phip]})
    structural_sim_indices.update({"sagittal_best_slice_phip": ssi_sag[max_snr_slice_sag_phip]})
    structural_sim_indices.update({"axial_best_slice_phip": ssi_ax[max_snr_slice_ax_phip]})
    ...
    ax[0, 0].scatter(range(phip_bssfp_data.shape[2]), structural_sim_indices["coronal"], marker='x', color='r')
    ax[0, 1].scatter(range(phip_bssfp_data.shape[1]), structural_sim_indices["sagittal"], marker='x', color='r')
    ax[0, 2].scatter(range(phip_bssfp_data.shape[0]), structural_sim_indices["axial"], marker='x', color='r')
    ...
    ax[1, 0].plot(range(dnp_bssfp_data.shape[2]), snr_per_slice_dnp["coronal"], label='DNP')
    ax[1, 1].plot(range(dnp_bssfp_data.shape[1]), snr_per_slice_dnp["sagittal"], label='DNP')
    ax[1, 2].plot(range(dnp_bssfp_data.shape[0]), snr_per_slice_dnp["axial"], label='DNP')

    ax[1, 0].plot(range(phip_bssfp_data.shape[2]), snr_per_slice_phip["coronal"], label='PHIP')
    ax[1, 1].plot(range(phip_bssfp_data.shape[1]), snr_per_slice_phip["sagittal"], label='PHIP')
    ax[1, 2].plot(range(phip_bssfp_data.shape[0]), snr_per_slice_phip["axial"], label='PHIP')

    ax[0, 0].scatter(max_snr_slice_cor_dnp, structural_sim_indices["coronal_best_slice_dnp"])
    ax[0, 0].scatter(max_snr_slice_cor_phip, structural_sim_indices["coronal_best_slice_phip"])

    ax[0, 1].scatter(max_snr_slice_sag_dnp, structural_sim_indices["sagittal_best_slice_phip"])
    ax[0, 1].scatter(max_snr_slice_sag_phip, structural_sim_indices["sagittal_best_slice_phip"])

    ax[0, 2].scatter(max_snr_slice_ax_dnp, structural_sim_indices["axial_best_slice_phip"])
    ax[0, 2].scatter(max_snr_slice_ax_phip, structural_sim_indices["axial_best_slice_phip"],
                     label='SSI from slice with highest SNR')

    ax[1, 0].vlines(max_snr_slice_cor_dnp, 0, np.max(snr_per_slice_dnp["coronal"]), linestyle='dashed', color='k')
    ax[1, 1].vlines(max_snr_slice_sag_dnp, 0, np.max(snr_per_slice_dnp["sagittal"]), linestyle='dashed', color='k')
    ax[1, 2].vlines(max_snr_slice_ax_dnp, 0, np.max(snr_per_slice_dnp["axial"]), linestyle='dashed', color='k')

    ax[1, 0].vlines(max_snr_slice_cor_phip, 0, np.max(snr_per_slice_phip["coronal"]), linestyle='dashed', color='k')
    ax[1, 1].vlines(max_snr_slice_sag_phip, 0, np.max(snr_per_slice_phip["sagittal"]), linestyle='dashed', color='k')
    ax[1, 2].vlines(max_snr_slice_ax_phip, 0, np.max(snr_per_slice_phip["axial"]), linestyle='dashed', color='k',
    ...
    return structural_sim_indices,mean_squared_errors,snr_per_slice_dnp,snr_per_slice_phip
